...
import type { StoredQQGuildMessage } from './types'

const STORAGE_DIR = 'qq-guild-data'
const MESSAGES_FILE = 'messages.json'

/**
 * QQ Guild message storage
 * Single-user mode: stores all messages in a flat list
 */
class QQGuildStorage {
  private storagePath: string
  private messages: StoredQQGuildMessage[] = []
  ...
  private async loadData(): Promise<void> {
    try {
      ...
      if (Array.isArray(data)) {
        this.messages = data as StoredQQGuildMessage[]
        console.log('[QQ Guild Storage] Loaded', this.messages.length, 'messages')
      } else {
        // Old format or corrupted, start fresh
        console.log('[QQ Guild Storage] Invalid data format, starting fresh')
        this.messages = []
      }
    } catch {
      // File doesn't exist yet
      this.messages = []
      console.log('[QQ Guild Storage] No existing messages found')
    }
  }
   ...
   * Store a message
   */
  async storeMessage(message: StoredQQGuildMessage): Promise<void> {
   ...
   */
  async getMessages(limit?: number): Promise<StoredQQGuildMessage[]> {
    await this.ensureInitialized()
...
export const qqGuildStorage = new QQGuildStorage()
